...
const Home = () => {
  ...
  useEffect(() => {
    const onFeaturedUpdated = () => {
      fetchFeaturedProperties();
    };

    const onStorage = (e) => {
      if (e.key === 'featuredUpdated') {
        fetchFeaturedProperties();
      }
    };

    window.addEventListener('featured-updated', onFeaturedUpdated);
    window.addEventListener('storage', onStorage);

    return () => {
      window.removeEventListener('featured-updated', onFeaturedUpdated);
